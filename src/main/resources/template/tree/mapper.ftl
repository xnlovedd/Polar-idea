...
<mapper namespace="${code.packageName}.${code.moduleName}.dao.${code.daoName}">
	...
	<#list columns as mData>
		<#if  mData.type!=10&&mData.type!=12>
			<#if mData.listReturn ==1>
			a.${mData.name} AS "${mData.javaName}",
			<#else >
				<#if mData.name == code.parentField ||  mData.name == code.childField>
			a.${mData.name} AS "${mData.javaName}",
				</#if>
			</#if>
		</#if>
...
${sql?substring(0, sql?last_index_of(","))}
	...
	<sql id="searchMultiWheres">
		<where>
			...
			a.${code.effectivenessField}=<#if code.effectivenessType ==0> '${code.effectivenessValue}'<#else>${code.effectivenessValue}</#if>
				</#if>
	...
	<sql id="orderCondition">
        order by a.${code.parentField} asc
	</sql>
...
${sql?substring(0, sql?last_index_of(","))}
    ...
    <!-- 查询全部集合 -->
    <select id="selectAllList" <#if code.moduleType==0 ||code.moduleType==2>resultType="java.util.Map"<#else>resultType="${code.entityAlias}"</#if>
            parameterType="java.util.Map">
        SELECT
        <include refid="listColumns" />
        FROM
