...
<#list children as child>
		request.setAttribute("${child.code.entityName?uncap_first}", "[]");
</#list>
	...
	public String updatePage(HttpServletRequest request, String ${code.idField}) {
		...
		request.setAttribute("${code.moduleName}", entity);
<#list children as child>
	<#if code.moduleType==0 || code.moduleType==2>
		request.setAttribute("${child.code.entityName?uncap_first}", GsonUtil.toJson(entity.get("${child.code.entityName?uncap_first}")));
	<#else >
		request.setAttribute("${child.code.entityName?uncap_first}", GsonUtil.toJson(entity.get${child.code.entityName?cap_first}()));
	</#if>
</#list>
		return "/view/modules/${code.moduleName}/${code.jspEditName}.jsp";
	}
...
<#list children as child>
	...
	@RequiresUser
	@RequiresPermissions(value = { "polar:backstage","${code.moduleName}:view:detail"})
	@ErrorMsg(tag = "查看${child.code.tableRemark}", type = ErrorType.JSON)
	@RequestMapping(value = "/${child.code.moduleName}Detail", produces = "text/html;charset=utf-8")
	public String ${child.code.moduleName}DetailPage(HttpServletRequest request,${child.code.entityName} entity) {
  		return ${child.code.moduleName}UpdatePage(request,entity);
	}

	/**
	 * 更新${child.code.moduleRemark}页面
	 *
	 * @param request
	 *            request对象
	 * @param entity
	...
	@RequiresUser
	@RequiresPermissions(value = { "polar:backstage","${code.moduleName}:edit"})
	@ErrorMsg(tag = "更新${child.code.tableRemark}", type = ErrorType.JSON)
	@RequestMapping(value = "/${child.code.moduleName}Update", produces = "text/html;charset=utf-8")
	public String ${child.code.moduleName}UpdatePage(HttpServletRequest request,${child.code.entityName} entity) {
		request.setAttribute("${child.code.moduleName}", entity);
		return "/view/modules/${code.moduleName}/${child.code.jspEditName}.jsp";
	}

	/**
	 * ${child.code.moduleRemark}新增页面
	 *
	 * @param request
	 *            request对象
     *
	...
	@RequiresUser
	@RequiresPermissions(value = { "polar:backstage","${code.moduleName}:add"})
	@ErrorMsg(tag = "新增${child.code.tableRemark}", type = ErrorType.JSON)
	@RequestMapping(value = "/${child.code.moduleName}Add", produces = "text/html;charset=utf-8")
	public String ${child.code.moduleName}Add(HttpServletRequest request) {
		return ${child.code.moduleName}UpdatePage(request,new ${child.code.entityName}());
	}
</#list>
}
