...
<div class="polar-img-${pageScope.name}">
	<c:if test="${not empty pageScope.value}">
		<c:forEach  items="${pageScope.value}" var='item' varStatus="status">
			<div class="polar-img-item-${pageScope.name}-${status.index} item" style="height: <c:choose><c:when test="${not empty pageScope.height}">${pageScope.height}</c:when><c:otherwise>80px</c:otherwise></c:choose>;width: <c:choose><c:when test="${not empty pageScope.width}">${pageScope.width}</c:when><c:otherwise>80px</c:otherwise></c:choose>;position: relative;float: left;margin-left: 2px;margin-right: 2px;">
				...
				<div class="polar-img" style="position: absolute; left: 0px;right: 0px;top: 0px;bottom: 0px;">
					<img class="polar-img-img" src="${item}" height="<c:choose><c:when test="${not empty pageScope.height}">${pageScope.height}</c:when><c:otherwise>80px</c:otherwise></c:choose>" width="<c:choose><c:when test="${not empty pageScope.width}">${pageScope.width}</c:when><c:otherwise>80px</c:otherwise></c:choose>" />
					<div class="polar-img-close" style="position: absolute;right:2px;top: 2px;"><i class="fa fa-close"></i></div>
				</div>
			</div>
		</c:forEach>
    ...
    ,error: function(index,obj,uuid){
     ...
     ,before:function(ojb,uuid){//上传前的回调
         if(${max}!=-1){
			 ...
			 if(count>${max}){
				 polar.dialog.alert("提示","最多只能上传${max}张图片");
				 return false;
			 }
         }
         //增加一个图片、加载框
