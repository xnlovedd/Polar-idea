...
<div class="input-group polar-tree-${pageScope.id}" >
	<input type="hidden" <c:if test="${not empty pageScope.name}">name="${pageScope.name}"</c:if> class="polar-tree-value" value="${value}"/>
...
$(function(){
	...
	if('${pageScope.value}'!=''){
		...
		$('.polar-tree-${pageScope.id} .polar-tree-display').val(mName);
	}
		...
		var options={yesBtn:true,title:title,yesContent:"确定",success:function(layero, index){
			...
			if(!polar.isNull(values)){
				...
				for(var position=0;position<tmps.length;position++){
					...
					var node=tree.getNodesByParam("${pageScope.valueField}",tmp);
