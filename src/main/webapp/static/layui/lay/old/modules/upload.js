...
layui.define('layer' , function(exports){
  ...
  Class.prototype.upload = function(files, type){
    ...
    ,ajaxSend = function(uuid){
      ...
      layui.each(items, function(index, file){
        ...
        ,progress=function(progress,uuid){
            if(options.progress!=null&& typeof options.progress === 'function'){
                options.progress(progress,uuid);
            }
        }
    //低版本IE处理方式，不支持跨域
    ,iframeSend = function(uuid){
    ...
    ,send = function(){
      ...
      var uuid=polar.uuid();
      //上传前的回调
        var  flag= options.before && options.before(args,uuid);
      if(flag){
          //IE兼容处理
          if(device.ie){
              return device.ie > 9 ? ajaxSend(uuid) : iframeSend(uuid);
          }
          ajaxSend(uuid);
      }
