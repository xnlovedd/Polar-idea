...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  //主模板
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" ' +
      'lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    if(options.url){ //Ajax请求
      ...
      params[request.limitName] = options.limit;
        ...
        var queryData=queryParams(pa);
      //参数
      var data = $.extend(queryData, options.where);
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              if(item3.toolbar){
                return laytpl($(item3.toolbar).html()||'').render(tplData);
                  ...
                  //@added增加格式化列方法
                  if(item3.formatter!=null&&item3.formatter!='undefined'){
                      try {
                          return item3.formatter(item1, options);
                      } catch (e) {
                          console.log("解析表列失败："+e);
                          return "";
                      }
                  }
  ...
  Class.prototype.events = function(){
    ...
    that.layBody.on('click', 'td', function(){
      ...
      if(Math.round(elemCell.prop('scrollWidth')) > Math.round(elemCell.outerWidth())){
       // @added修改bug
          Class.prototype.tipsIndex = layer.tips([
