...
<div class="input-group polar-tree-userRole" >
	<ul class="polar-tree-ul-userRole ztree"></ul>
</div>
<script>
	$(function(){
		var obj={
			...
			init:function(){
				var data=[
					<c:forEach items="${allRoles}" var='dict' varStatus="stat">
			           {id:"${dict.id}",parentId:"${dict.parentId}",text:"${dict.text}" } <c:if test="${!stat.last}">,</c:if>
			        </c:forEach>
				];
				var checkData=[
					<c:forEach items="${userRoles}" var='dict' varStatus="stat">
			           {id:"${dict.id}",parentId:"${dict.parentId}",text:"${dict.text}"} <c:if test="${!stat.last}">,</c:if>
			        </c:forEach>
				];
				var setting = {
				...
				var nodes =data;
				var tree=$.fn.zTree.init($(".polar-tree-ul-userRole"), setting, nodes);
				this.tree=tree;
				tree.expandAll(true);
