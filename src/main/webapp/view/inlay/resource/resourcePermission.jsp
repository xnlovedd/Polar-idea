...
<div class="input-group polar-tree-userRole" >
	<ul class="polar-tree-ul-resourcePermission ztree"></ul>
</div>
<script>
	$(function(){
		var obj={
			...
			init:function(){
				...
				var checkData=[
					<c:forEach items="${resourcePermissions}" var='dict' varStatus="stat">
			           {id:"${dict.id}",parentId:"${dict.parentId}",text:"${dict.text}"} <c:if test="${!stat.last}">,</c:if>
			        </c:forEach>
				];
				var setting = {
				...
				var nodes =data;
				var tree=$.fn.zTree.init($(".polar-tree-ul-resourcePermission"), setting, nodes);
				this.tree=tree;
				tree.expandAll(true);
			...
			,submit:function(index){
				var url="/resource/json/updateResourcePermissions";
